...
def test_default_patrones_incluir_is_match_all():
    ...
    assert transcriptor4ai.filtering.matches_include("anything.py", compiled) is True
...
def test_matches_any_true_false_and_empty_list():
    compiled = transcriptor4ai.filtering.compile_patterns([r"^a.*", r".*\.py$"])

    assert transcriptor4ai.filtering.matches_any("abc", compiled) is True
    assert transcriptor4ai.filtering.matches_any("file.py", compiled) is True
    assert transcriptor4ai.filtering.matches_any("zzz", compiled) is False
    assert transcriptor4ai.filtering.matches_any("anything", []) is False


def test_matches_include_false_when_include_empty():
    assert transcriptor4ai.filtering.matches_include("file.py", []) is False
...
def test_es_test_edge_cases():
    ...
    assert transcriptor4ai.filtering.es_test("_test.py") is True  # matches .*_test.py
    assert transcriptor4ai.filtering.es_test("test.py") is False  # no underscore after test
    assert transcriptor4ai.filtering.es_test("mytest_file.py") is False


def test_safe_mkdir_creates_and_ok_if_exists(tmp_path):
    target = tmp_path / "out"
    ok, err = transcriptor4ai.paths._safe_mkdir(str(target))
    assert ok is True
    assert err is None
    assert target.exists() and target.is_dir()

    # Calling again should still be ok
    ok2, err2 = transcriptor4ai.paths._safe_mkdir(str(target))
    assert ok2 is True
    assert err2 is None
