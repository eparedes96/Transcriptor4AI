...
def test_default_extensiones_is_py():
    assert tg._default_extensiones() == [".py"]


def test_default_patrones_incluir_is_match_all():
    pats = tg._default_patrones_incluir()
    assert pats == [".*"]
    compiled = tg._compile_patterns(pats)
...
def test_default_patrones_excluir_includes_pycache_and_hidden():
    pats = tg._default_patrones_excluir()
    assert any("__pycache__" in p for p in pats)
    assert any(r"^\." in p for p in pats)

    compiled = tg._compile_patterns(pats)
    assert tg._matches_any("__pycache__", compiled) is True
    assert tg._matches_any(".venv", compiled) is True
    assert tg._matches_any("normal_dir", compiled) is False


def test_compile_patterns_keeps_valid_drops_invalid():
    patterns = [r"^ok$", r"([unclosed", r".*\.py$"]
    compiled = tg._compile_patterns(patterns)
...
def test_matches_any_semantics_match_not_search():
    compiled = tg._compile_patterns([r"abc"])
    # re.match requires match at the start; "xabc" should NOT match
    assert tg._matches_any("abc", compiled) is True
    assert tg._matches_any("xabc", compiled) is False


def test_matches_any_empty_list_is_false():
    assert tg._matches_any("anything", []) is False


def test_es_test_same_behavior_as_transcriptor():
    assert tg._es_test("test_algo.py") is True
    assert tg._es_test("algo_test.py") is True

    assert tg._es_test("algo.py") is False
    assert tg._es_test("test_algo.txt") is False
    assert tg._es_test("algo_test.txt") is False


def test_es_test_edge_cases():
    assert tg._es_test("test_.py") is True
    assert tg._es_test("_test.py") is True
    assert tg._es_test("test.py") is False
    assert tg._es_test("mytest_file.py") is False
