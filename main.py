...
def main() -> None:
    ...
    # -------------------------
    config = core.cargar_configuracion()
    ...
    while True:
        ...
        if event == "btn_explorar_in":
            initial = core.normalizar_dir(values.get("ruta_carpetas", ""), os.path.dirname(os.path.abspath(__file__)))
        ...
        if event == "btn_examinar_out":
            initial = core.normalizar_dir(values.get("output_base_dir", ""), values.get("ruta_carpetas", ""))
        ...
        if event == "Guardar Configuración":
            try:
                ...
                config["ruta_carpetas"] = core.normalizar_dir(config["ruta_carpetas"], os.path.dirname(os.path.abspath(__file__)))
                config["output_base_dir"] = core.normalizar_dir(config["output_base_dir"], config["ruta_carpetas"])
                core.guardar_configuracion(config)
        ...
        if event == "Resetear Configuración":
            config = core.cargar_configuracion_por_defecto()
        ...
        if event == "Procesar":
            try:
                ...
                # Normalizar rutas
                ruta_carpetas = core.normalizar_dir(config["ruta_carpetas"], os.path.dirname(os.path.abspath(__file__)))
                output_base_dir = core.normalizar_dir(config["output_base_dir"], ruta_carpetas)
                output_subdir_name = (config.get("output_subdir_name") or "").strip() or core.DEFAULT_OUTPUT_SUBDIR
                output_prefix = (config.get("output_prefix") or "").strip() or core.DEFAULT_OUTPUT_PREFIX
                ...
                salida_real = core.ruta_salida_real(output_base_dir, output_subdir_name)
                ...
                nombres = core.archivos_destino(output_prefix, modo_final, incluir_arbol)
                existentes = core.existen_ficheros_destino(salida_real, nombres)
